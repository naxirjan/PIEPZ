...
$(document).ready(function (){
  ...
  let objParams = {
    ...
    sInfos: "",
    aRowsCount: [-1],
    sAll: "Editing "+((iTotal == undefined || iTotal == 'undefined' || iTotal == "" || iTotal == null) ? "" : iTotal)+" products",
  };
  ...
  objBootGrid.on("loaded.rs.jquery.bootgrid", function (e) {
    ...
    objBootGrid.on("change", "input.name", function () {
      e.stopPropagation();
  ...
  $(document).on("click", "#btnSave", function () {
    ...
    if(aData.length > 0) {
      //Ajax Call
      callAJAXRequest("/admin/bulk-edit-products-operations", params);
    }
  });
...
function getUpdatedData($this, aData, sTableSelector) {
  ...
  let sName = $("#"+iRowId+"-name-"+sSku).val();
  ...
  if (iRowId != null) {
    ...
    for (let i= 0; i < iColumnsLength; i++){
      ...
      if(sColumnName == "product") aFieldsData.name = sName;
