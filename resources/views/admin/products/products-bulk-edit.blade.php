...
@section('page-script')
  ...
  <script src="{{asset('js/admin/jsProductBulkEdit.js')}}"></script>
@endsection
...
@section('content')
  ...
  <div class="row">
    <div class="col-md-12">
      ...
      -->
      <table id="gridBulkEditProducts" data-total="{{count($aProducts)}}" class="table table-striped table-responsive" style="table-layout: auto;">
        <thead>
        <tr>
          <th
            data-column-id="sno"
        ...
        @if(count($aProducts))
          @foreach($aProducts as $iKey => $product)
            <tr>
              <td>{{($iKey+1)}}</td>
              <td><img src='{{asset("storage")}}/{{$product->image}}' alt='No'/></td>
              <td><input style="width: auto" type="text" class="name form-control" id="{{$product->id}}-name-{{$product->sku}}" value="{{$product->name}}"></td>
              <td>
