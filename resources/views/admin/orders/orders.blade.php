...
@section('page-script')
  ...
  <script src="{{asset('js/admin/jsOrders.js')}}"></script>
@endsection
...
@section('content')
  <h4 class="fw-bold py-3 mb-4">
    ...
    <a href="/admin/create-new-order" type="button" style="float:right;" class="btn rounded-pill btn-icon btn-label-primary waves-effect" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-original-title="Create New Order">
  ...
  <div class="row">
    <div class="col-sm-12">
      <ul class="nav nav-tabs widget-nav-tabs pb-1 gap-2 mx-1 d-flex flex-nowrap" role="tablist">
        <li class="nav-item">
          <a style="width: 90px !important; height: auto !important;" href="/admin/orders/{{base64_encode(0)}}"
        ...
        <li class="nav-item">
          <a style="width: 140px !important; height: auto !important;" href="/admin/orders/{{base64_encode(1)}}"
        ...
        <li class="nav-item">
          <a style="width: 135px !important; height: auto !important;" href="/admin/orders/{{base64_encode(2)}}"
        ...
        <li class="nav-item">
          <a style="width: 130px !important; height: auto !important;" href="/admin/orders/{{base64_encode(3)}}"
          ...
          <table id="gridOrders" class="table  table-hover"
            ...
            @if(count($aOrders))
              @foreach($aOrders as $iKey => $aOrder)
                <tr>
                  <td><b>{{($iKey+1)}}</b></td>
                  <td><span class="badge badge-xs {{($aOrder->status == 'Open' ? 'bg-warning' : ($aOrder->status == 'Completed' ? 'bg-success' : 'bg-danger'))}}">{{$aOrder->status}}</span></td>
                  <td>{{$aOrder->vendor}}</td>
                  <td>{{$aOrder->order_number}}</td>
                  <td>{{$aOrder->quantity}}</td>
                  <td>{{$aOrder->partner}}</td>
                  <td>{{$aOrder->total_price}}</td>
                  <td>{{$aOrder->tracking_code}}</td>
                  <td>{{date("d F Y", strtotime($aOrder->created_at))}}</td>
                  <td><a href="/admin/order/details/{{base64_encode($aOrder->id)}}" class="badge bg-info">View</a> </td>
                </tr>
              @endforeach
            @endif
            </tbody>
